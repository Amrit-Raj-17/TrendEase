...
function Navbar() {
  ...
  return (
    <div className='navbar flex items-center justify-between py-1 px-4 bg-gradient-to-r from-purple-50 via-pink-200 to-blue-200 shadow-sm font-serif font-bold'>
      <Link to={'/'}><img src={assets.logo_2} alt="" className='logo w-8 sm:w-10 md:w-12 lg:w-14 object-contain'/></Link>
      ...
      <div className='flex items-center gap-6'>
           <img onClick={()=>setShowSearch(true)} src={search_icon} alt="" className='w-7 cursor-pointer'/>
           ...
           <Link to='/cart' className='relative'>
               <img src={cart_icon} alt="" className='w-7 min-w-5' />
               <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>{getCartCount()}</p>
           </Link>
           <img onClick={()=>setVisible(true)} src={menu_icon} alt="" className='w-7 cursor-pointer sm:hidden' />
        </div> 



        {/**Sidebar menu for small screens */}
          <div className={`fixed top-0 right-0 bottom-0 z-50 bg-gradient-to-b from-blue-100 to-white shadow-2xl transition-all duration-300 ease-in-out ${visible ? 'w-3/4 sm:w-1/2 px-5' : 'w-0 overflow-hidden'}`}>
          <div className='flex flex-col h-full text-gray-800 font-medium py-6 space-y-4'>

          {/* Back Button */}
          <div onClick={() => setVisible(false)} className='flex items-center gap-3 p-3 cursor-pointer text-sm text-blue-700 hover:bg-blue-200 rounded-md transition'>
            <FaArrowLeft />
            <p>Back</p>
          </div>
